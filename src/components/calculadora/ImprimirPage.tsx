...
export function ImprimirPage({ id }: { id: string }) {
  ...
  if (estado === "cargando") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F5F1E8]">
        <p className="text-[#1F3A38] font-bold text-sm">Cargando documento...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#ECE7D8]">
      <style>{`
        @media print {
      ...
      <div className="no-imprimir sticky top-0 z-10 bg-[#1F3A38] text-white px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[#C97B3D] hover:bg-[#B36A2F] px-4 py-2 rounded-lg text-sm font-bold transition-colors"
          >
      ...
      <div ref={docRef} className="max-w-[8.5in] mx-auto bg-white my-6 shadow-lg">
        <div className="bg-[#1F3A38] px-8 py-6 flex items-center gap-4">
          ...
          <div className="text-white leading-tight">
            ...
            <p className="text-sm font-bold text-[#9FBFB9]">
        ...
        <div className="px-8 py-6 space-y-5">
          <div>
            <h1 className="text-2xl font-bold text-[#1F3A38]">Cotización</h1>
            <p className="text-xs font-bold text-[#8A836C]">
              {fecha} · {etiquetaTipoCliente(datos.tipo_cliente)}
            </p>
          </div>

          {tieneCliente && cliente && (
            <div className="border border-[#E4DDC9] rounded-lg px-4 py-3">
              <p className="text-xs uppercase font-bold text-[#8A836C] mb-2">Datos del cliente</p>
          ...
          <div>
            <p className="text-xs uppercase font-bold text-[#8A836C] mb-2">Productos cotizados</p>
            <div className="space-y-2">
              {calculados.map(({ nombre, imagen, descripcion, calc }, i) => {
                ...
                return (
                  <div key={i} className="border border-[#E4DDC9] rounded-lg px-3 py-2.5 flex items-start gap-3">
                    {imagen && (
                      <img
                        src={imagen}
                        alt={nombre || `Producto ${i + 1}`}
                        className="w-16 h-16 rounded-md object-cover border border-[#E4DDC9] shrink-0"
                      />
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between flex-wrap gap-1">
                        <span className="text-sm font-bold text-[#1F3A38]">{nombre || `Producto ${i + 1}`}</span>
                        <span className="text-xs font-bold text-[#6B6552]">
                          Contado {fmt(precioFinal)} · Crédito {fmt(precioCredito)}
                        </span>
                      </div>
                      {descripcion && (
                        <p className="text-[11px] text-[#6B6552] mt-1 whitespace-pre-wrap">{descripcion}</p>
                      )}
                    </div>
                  </div>
                );
          ...
          <div className="bg-[#1F3A38] text-white rounded-xl px-5 py-4 text-center">
            <p className="text-[10px] uppercase font-bold text-[#9FBFB9]">Total pago al contado</p>
            <p className="text-2xl font-bold">{fmt(contadoTotal)}</p>
          </div>

          <div>
            <p className="text-xs uppercase font-bold text-[#8A836C] mb-2">Plazos disponibles a crédito (total {fmt(creditoTotal)})</p>
            <div className="grid grid-cols-2 gap-2">
              {totales.planTotal.map((row) => (
                <div key={row.meses} className="border border-[#E4DDC9] rounded-lg px-3 py-2">
                  <p className="text-xs font-bold text-[#1F3A38]">{row.meses} meses</p>
                  <p className="text-sm font-bold text-[#1F3A38]">{fmt(row.cuotaMensual)}/mes</p>
                  <p className="text-[10px] font-bold text-[#8A836C]">{fmt(row.letraQuincenal)} quincenal</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {tipo_cliente_asociado && (
            <div className="rounded-xl overflow-hidden border-2 border-[#C97B3D]">
              ...
              <div className="bg-[#FBEDE0] px-4 py-3 text-center">
                <p className="text-[10px] uppercase font-bold text-[#9C5A24]">Precio de etiqueta</p>
                <p className="text-xl font-bold text-[#1F3A38] mb-2">{fmt(totales.precioContado)}</p>
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-white rounded-lg border border-[#E9CDAE] px-3 py-2">
                    <p className="text-[10px] uppercase font-bold text-[#9C5A24]">Abono inicial</p>
                    <p className="text-sm font-bold text-[#1F3A38]">{fmt(totales.cuotaPromoContado)}</p>
                  </div>
                  <div className="bg-white rounded-lg border border-[#E9CDAE] px-3 py-2">
                    <p className="text-[10px] uppercase font-bold text-[#9C5A24]">Quincenal</p>
                    <p className="text-sm font-bold text-[#1F3A38]">{fmt(totales.cuotaPromoContado / 2)}</p>
                  </div>
                </div>
                <p className="text-[10px] text-[#9C5A24] mt-1.5">Sujeto a capacidad comprobada</p>
              </div>
            </div>
          )}
          ...
          {capacidad && (
            ...
            >
              ...
              <p className="text-[10px] text-[#8A836C] mt-2 leading-relaxed">
                Aplica el tope legal panameño de endeudamiento vigente. Sujeto a confirmación final por la cooperativa.
              </p>
            </div>
          )}

          <div className="bg-[#FBEDE0] text-[#9C5A24] rounded-full text-center py-2 text-xs font-bold">
            Cotización válida por 30 días o hasta agotar existencias
          </div>

          <p className="text-center text-xs font-bold text-[#8A836C]">WhatsApp Línea Blanca: +507 6784-1941</p>
        </div>
      </div>
    </div>
  );
...
function InfoLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between gap-2">
      ...
      <span className="font-bold text-[#1F3A38] text-right">{value}</span>
    </div>
  );
}
